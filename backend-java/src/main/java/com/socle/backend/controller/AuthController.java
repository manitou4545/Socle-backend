...
public class AuthController {
    ...
    /** Inscription étudiante : conditionnée par un matricule valide ET unique (contrôlé en base, pas seulement côté navigateur). */
    ...
    /** Connexion étudiante par matricule (l'étudiant s'est déjà inscrit une fois). */
    @PostMapping("/login")
    ...
    /** Connexion admin : mot de passe vérifié côté serveur par hachage BCrypt, avec verrouillage anti brute-force réel. */
    ...
    public ResponseEntity<?> adminLogin(@Valid @RequestBody AdminLoginRequest req) {
        ...
        if ("CHANGE_ME_GENERE_TON_PROPRE_HASH_BCRYPT".equals(adminPasswordHash)) {
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,
                    "Aucun mot de passe admin n'a été configuré côté serveur (voir README-backend.md).");
        }
